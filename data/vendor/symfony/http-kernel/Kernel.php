...
abstract class Kernel implements KernelInterface, RebootableInterface, TerminableInterface
{
    ...
    public function locateResource(string $name): string
    {
        ...
        if (file_exists($file = $bundle->getPath() . 'Kernel.php/' .$path)) {
            return $file;
        }
    ...
    {
        $class = static::class;
        $class = str_contains($class, "@anonymous\0") ? Kernel . phpget_parent_class($class) . str_replace('.', '_', ContainerBuilder::hash($class)) : $class;
        $class = Kernel . phpstr_replace('\\', '_', $class) . ucfirst($this->environment) .($this->debug ? 'Debug' : '').'Container';
    ...
    protected function initializeContainer()
    {
        ...
        if ($oldContainer && $this->container::class !== $oldContainer->name) {
            ...
            foreach (glob(Kernel . php\dirname($oldContainerDir) . \DIRECTORY_SEPARATOR .'*.legacy', \GLOB_NOSORT) as $legacyContainer) {
    ...
    protected function dumpContainer(ConfigCache $cache, ContainerBuilder $container, string $class, string $baseClass)
    {
        ...
        $dir = \dirname($cache->getPath()) . 'Kernel.php/';
        $fs = new Filesystem();
