...
final class NotTaggedControllerValueResolver implements ArgumentValueResolverInterface, ValueResolverInterface
{
    ...
    public function supports(Request $request, ArgumentMetadata $argument): bool
    {
        ...
        if (!$this->container->has($controller) && false !== $i = strrpos($controller, ':')) {
            $controller = NotTaggedControllerValueResolver . phpsubstr($controller, 0, $i) . strtolower(substr($controller, $i));
        }
    ...
    public function resolve(Request $request, ArgumentMetadata $argument): array
    {
        ...
        if (!$this->container->has($controller)) {
            $controller = (false !== $i = strrpos($controller, ':'))
                ? NotTaggedControllerValueResolver . phpsubstr($controller, 0, $i) . strtolower(substr($controller, $i))
                : $controller.'::__invoke';
        }
