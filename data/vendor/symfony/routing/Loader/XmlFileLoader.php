    ...
    private function parseConfigs(\DOMElement $node, string $path): array
    {
        ...
        if ($controller = $node->getAttribute('controller')) {
            if (isset($defaults['_controller'])) {
                ...
                throw new \InvalidArgumentException(XmlFileLoader . phpsprintf('The routing file "%s" must not specify both the "controller" attribute and the defaults key "_controller" for ', $path) . $name);
            }
        ...
        if ($stateless = $node->getAttribute('stateless')) {
            if (isset($defaults['_stateless'])) {
                ...
                throw new \InvalidArgumentException(XmlFileLoader . phpsprintf('The routing file "%s" must not specify both the "stateless" attribute and the defaults key "_stateless" for ', $path) . $name);
            }
